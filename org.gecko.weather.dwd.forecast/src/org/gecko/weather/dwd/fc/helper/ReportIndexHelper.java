...
public class ReportIndexHelper {
	...
	/**
	 * Maps a given {@link WeatherReport} to {@link ObjectContextObject} with a given {@link IndexActionType}
	...
	public static EObjectDocumentIndexObjectContext mapReport(WeatherReport report, IndexActionType indexAction) {
		...
		reportAttributes.
			stream().
			filter(a->a.getEAttributeType().getInstanceClass() == Float.class).
			forEach(a->{
	...
	public static WeatherReport mapDocument(Document document, ResourceSet rs) {
		...
		reportAttributes.forEach(a->{
			getValue(document, a.getName()).
				map(Float.class::cast).
				ifPresent(fv->r.eSet(a, fv));
		});
		return r;
	}

...
/**
 * Command to search for station by name or by location:
 * stationByName jena
 * stationByLocation -p|--position 50.892691,12.267116 (-r|--radius 10)
 * - the -p or --position parameter is mandatory whereas the radius can be omitted and defaults to 30 km 
 * @author Mark Hoffmann
 * @since 10.09.2024
 */
@Component (service = Object.class, reference = {
...
public class StationSearchCommand {
	...
	public void stationByName(@Descriptor("The station name to search for") String name) {
		...
		byName.forEach(s->{
			...
			System.out.println(String.format("[%s] Name: '%s', Latitude: %s, Longitude: %s", s.getId(), s.getName(), lat, lon));
		});
	}
	
	/**
	 * Search a station by location like this:
	 * <b>stationbylocation -p 50.892691,12.267116 -r 30</b>
	 * <b>stationbylocation --position 50.892691,12.267116</b>
	...
	public void stationByLocation(@Parameter(names={"-p", "--position"}, absentValue = "n/a,n/a") @Descriptor("Geo-position in 'lat, lon'") GeoPosition position,@Parameter(names={"-r", "--radius"}, absentValue = "30") @Descriptor("The search radius in km (optional)") Integer radius) {
		if (isNull(position)) {
		...
		System.out.println(String.format("Searching for stations near location 'lat: %s, lon: %s' within a radius of %s km", position.getLatitude(), position.getLongitude(), radius));
